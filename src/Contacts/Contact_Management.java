  ...
  ResultSet rs;
   ...
   public Contact_Management(){
     ...
     b.add(dataList);
      ...
      if(!this.connProcess()){		// 연락처 창 띄웠을때 DB로 부터 정보 불러와 Table 세팅
    	  System.out.println("서버와 접속 실패하였습니다.");
    	  return;
      } else {
    	  this.sqlQuery("SELECT * FROM contact_tbl");
   ...
   // TODO Auto-generated method stub
      ...
      if (obj instanceof JButton) {
         if (e.getSource() == insertBtn) {
        	 ...
        	 String sql = "INSERT INTO contact_tbl(name,phone,email) VALUES("+ 
        			 		"'" + this.textfields[0].getText() + "'," +
        			 		"'" + this.textfields[1].getText() + "'," +
        			 		"'" + this.textfields[2].getText() + "')";
        	 if (sqlUpdate(sql)) {
        		 sqlQuery("SELECT * FROM contact_tbl");
        	 ...
        	 String key_id = (String)model.getValueAt(row, 0);
        	 String sql = "DELETE FROM contact_tbl WHERE id = " + key_id + "";
        	 if (sqlUpdate(sql)) {
        		 sqlQuery("SELECT * FROM contact_tbl");
   ...
   private void update() {
      ...
      arr[0] = row + "";
      arr[1] = textfields[0].getText();
      arr[2] = textfields[1].getText();
      arr[3] = textfields[2].getText();
